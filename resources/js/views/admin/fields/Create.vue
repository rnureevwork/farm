<template>
    <div>
        <v-container fluid>
            <v-row>
                <v-col cols="12">
                    <h1 class="text-h4 mb-4">Добавить поле</h1>
                </v-col>
            </v-row>
            ...
            <v-row>
                <v-col cols="12" md="8">
                    <v-card>
                        <v-card-text>
                            <v-form @submit.prevent="saveField" ref="form">
                                <v-row>
                                    <v-col cols="12" md="6">
                                        <v-text-field
                                            v-model="form.name"
...
export default {
    ...
    async mounted() {
        ...
        // Если передан farm_id в query параметрах, устанавливаем его
        if (this.$route.query.farm_id) {
            this.form.farm_id = parseInt(this.$route.query.farm_id)
        }
    },
    methods: {
        async fetchFarms() {
        ...
        async saveField() {
            ...
            try {
                ...
                await axios.post('/api/v1/fields', data)
