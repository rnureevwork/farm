<template>
    <div>
        <v-container fluid>
            <v-row>
                <v-col cols="12">
                    <h1 class="text-h4 mb-4">Редактировать ферму</h1>
                </v-col>
            </v-row>
            ...
            <v-row>
                <v-col cols="12" md="8">
                    <v-card>
                        <v-card-text>
                            <v-form @submit.prevent="updateFarm" ref="form">
                                <v-row>
                                    <v-col cols="12" md="6">
                                        <v-text-field
                                            v-model="form.name"
    ...
    methods: {
        async fetchFarm() {
            try {
                ...
                this.form.contact_phone = response.data.contact_phone || ''
            } catch (error) {
        ...
        async updateFarm() {
            ...
            this.loading = true
            try {
                await axios.put(`/api/v1/farms/${this.$route.params.id}`, this.form)
                this.$router.push({ name: 'farms.index' })
            } catch (error) {
                console.error('Ошибка обновления фермы:', error)
                alert('Ошибка при обновлении фермы')
            } finally {
                this.loading = false
            }
        }
